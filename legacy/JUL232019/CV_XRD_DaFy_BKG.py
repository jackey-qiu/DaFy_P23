...
for scan_no in scan_nos:
    ...
    for frame_number in frame_number_ranges:
        ...
        if bkg_sub.int_direct =='y':
            bkg_sub.fit_background(None,img*mask,peak_width_hor_ver['ver'],plot_live = False)
        elif bkg_sub.int_direct =='x':
            bkg_sub.fit_background(None,img*mask,peak_width_hor_ver['hor'],plot_live = False)
        ...
        #launch tweak mode if the check_result is False
        if (not check_result) and (not mpi_run):
            tweak_mode, live_image = True, True
        else:
            tweak_mode, live_image = False, False
        ...
        if tweak_mode:
            ...
            peak_width_hor_ver = peak_fit_engine.get_peak_width()
            if bkg_sub.int_direct =='y':
                bkg_sub.fit_background(fig_bkg_plot,img*mask, data, plot_live = True)
            elif bkg_sub.int_direct =='x':
                bkg_sub.fit_background(fig_bkg_plot,img*mask, data, plot_live = True)
            peak_fit_engine.update_peak_intensity(bkg_sub.fit_results['I'])
    ...
    if rank == 0:
        ...
        if len(scan_nos)==1 and (not bool(debug)):
