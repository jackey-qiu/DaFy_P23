...
which_scans_to_plot = [742, 745, 746, 749]
which_scans_to_plot = [724, 727, 734][0:3]
config_file_name = ['CV_XRD_plot_I20180114.ini'][0]
...
#specify all limits
xlim = [0.25, 2.15]
ylim_ip_strain = [-0.7, 0.05]
ylim_oop_strain = [-1.1, 0.05]
ylim_ip_size = [-7, 0.05]
ylim_oop_size = [-8, 0.05]
...
def select_cycle(pot_result_couple, bin_level = bin_level, bin_mode = 'average', return_cycle = 1, pot_step = 0.10, plot_mode='CV'):
    ...
    if bin_level>1:
        ...
        return [0, int((index_of_valley_pot[return_cycle+1]-index_of_valley_pot[return_cycle])/2),-2],pot_bin, result_bin
    else:
...
if(1):
    ...
    for scan_id in scan_ids:
        ...
        if overplot:
            ...
            axs_all = [fig_all.add_subplot(5,len(scan_ids),i) for i in range(scan_ids.index(scan_id)+1,scan_ids.index(scan_id)+1+len(scan_ids)*5,len(scan_ids))]
            ...
            for ax in axs_all:
                ...
                if plot_pot_step:
                    if ax==axs_all[-1]:
                        ax.set_xlabel('Times(s)', fontsize=15)
                    else:
                        ax.set_xticklabels([])
                    if scan_id == scan_ids[0]:
        ...
        for key in data_dic.keys():
            if not plot_pot_step and data_dic[key].shape == np.array(potential_original).shape:
                scan_direction_ranges,temp,data_dic[key] = select_cycle((potential_original, data_dic[key]),plot_mode='CV')
            elif plot_pot_step and data_dic[key].shape == np.array(potential_original).shape:
                scan_direction_ranges,temp,data_dic[key] = select_cycle((potential_original, data_dic[key]),plot_mode='pot_step')
        data = data_dic
        ...
        for pos in range(len(scan_direction_ranges)):
            ...
            if plot_pot_step:
                label=scan_info[scan_id].scan_label
                color =scan_info[scan_id].color
            else:
                if(pos == 0):
                    label=scan_info[scan_id].scan_label+' positive scan'
                    me_color=scan_info[scan_id].color
                else:
            ...
            if plot_pot_step:
                marker = None
                if len(data['Time'])==0:
                    data['Time']=np.array(range(len(data['potential'])))
                ...
                if scan_id in ['P23config_zap_746_20','P23config_zap_746_100']:
                    offset =2
                ax1a.plot(data['Time'][indx1:indx2]*scale_factor+offset, set_max_to_0(strain_ip,[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                ax1_all.plot(data['Time'][indx1:indx2]*scale_factor+offset, set_max_to_0(strain_ip,[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                ax1_all.set_ylim(ylim_ip_strain)
                try:
                    ax_fig_ALL_strain_ip.plot(data['Time'][indx1:indx2]*scale_factor+offset, set_max_to_0(strain_ip,[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                    ax_fig_ALL_strain_ip.set_ylim(ylim_ip_strain)
                except:
                    pass
                ax2a.plot(data['Time'][indx1:indx2]*scale_factor+offset, set_max_to_0(strain_oop,[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                ax2_all.plot(data['Time'][indx1:indx2]*scale_factor+offset, set_max_to_0(strain_oop,[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                ax2_all.set_ylim(ylim_oop_strain)
                try:
                    ax_fig_ALL_strain_oop.plot(data['Time'][indx1:indx2]*scale_factor+offset, set_max_to_0(strain_oop,[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                    ax_fig_ALL_strain_oop.set_ylim(ylim_oop_strain)
                except:
                    pass
                ax3a.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(data['FWHM_ip'],[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                ax4a.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(data['FWHM_oop'],[indx1,indx2]), '-', linewidth=1,  color=color, marker=marker, label=label)
                ax5a.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(data['amp_ip']/data['amp_ip'][0],[indx1,indx2]), '-', linewidth=1,  color=color, marker=marker, label=label)
                try:
                    ax6_all.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(data['amp_ip']/data['amp_ip'][0],[indx1,indx2]), '-', linewidth=1,  color=color, marker=marker, label=label)
                except:
                    pass
                ax6a.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(0.2*np.pi/data['FWHM_ip'],[indx1,indx2]), 'o-', linewidth=1, color=color, marker=marker, label=label)
                ax4_all.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(0.2*np.pi/data['FWHM_ip'],[indx1,indx2]), 'o-', linewidth=1, color=color, marker=marker, label=label)
                ax4_all.set_ylim(ylim_ip_size)
                try:
                    ax_fig_ALL_size_ip.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(0.2*np.pi/data['FWHM_ip'],[indx1,indx2]), 'o-', linewidth=1, color=color, marker=marker, label=label)
                    ax_fig_ALL_size_ip.set_ylim(ylim_ip_size)
                except:
                    pass
                ax7a.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(0.2*np.pi/data['FWHM_oop'],[indx1,indx2])+0.25, '-', linewidth=1, color=color, marker=marker, label='')
                ax5_all.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(0.2*np.pi/data['FWHM_oop'],[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                ax5_all.set_ylim(ylim_oop_size)
                try:
                    ax_fig_ALL_size_oop.plot(data['Time'][indx1:indx2]*scale_factor+offset,set_max_to_0(0.2*np.pi/data['FWHM_oop'],[indx1,indx2]), '-', linewidth=1, color=color, marker=marker, label=label)
                    ax_fig_ALL_size_oop.set_ylim(ylim_oop_size)
                except:
                    pass
                ax9a.plot(data['Time'][indx1:indx2]*scale_factor+offset, POT(data['potential'][indx1:indx2], plot_vs_RHE, scan_info[scan_id].pH), '-', linewidth=1, color=color, marker=marker, label=label)
                ax3_all.plot(data['Time'][indx1:indx2]*scale_factor+offset, POT(data['potential'][indx1:indx2], plot_vs_RHE, scan_info[scan_id].pH), '-', linewidth=1, color=color, marker=marker, label=label)
                ...
                if scan_info[scan_id].ids_filename==None:
                    ...
                    ax3_all.set_ylim(ylim_current_density)
                else:
