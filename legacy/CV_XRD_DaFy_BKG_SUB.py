...
conf_file_names = [ "config_p23.ini","config_id03_MA4171.ini","CV_XRD_plot_temp.ini","config_id03_ch5314.ini","config_id03_ma3886.ini","config_bkg_sub.ini"]
conf_file = os.path.join(DaFy_path, 'config', conf_file_names[1])
conf_file_plot = os.path.join(DaFy_path, 'config', conf_file_names[2])
conf_file_bkg_sub = os.path.join(DaFy_path, 'config', conf_file_names[-1])
...
for scan_no in scan_nos:
    ...
    lattice_skin = rsp.lattice.from_cif(os.path.join(DaFy_path, 'util','cif',"{}".format(film_material_cif)), HKL_normal=[1,1,1],HKL_para_x=[1,1,-2], offset_angle = 15)
    ...
    bkg_sub = background_subtraction_single_img(conf_file_bkg_sub)
    ...
    try:
        peak_fit_engine = XRD_Peak_Fitting(img = grid_intensity_init, mask = mask, q_ip = grid_q_ip, q_oop=grid_q_oop,\
                                           ...
                                           peak_center = [cen[1],cen[0]],prim_beam_pot = [prim_beam_pot[1], prim_beam_pot[0]],\
    ...
    if debug:
        if type(debug_img)!=type([]):
            frame_number_ranges = [debug_img]
        elif len(debug_img)==2:
            frame_number_ranges = range(debug_img[0],debug_img[1])
    ...
    for frame_number in frame_number_ranges:
        ...
        if l_scan:#this step is time consuming for recal q, but only for L_scan
            ...
            q_oop, q_ip, grid_q_oop, grid_q_ip, img = r_map.q['q_perp'], r_map.q['q_par'],\
                                                      r_map.q['grid_q_perp'], r_map.q['grid_q_par'], r_map.grid_intensity
            mask = create_mask(img = img,img_q_par = grid_q_oop, img_q_ver = grid_q_ip,\
                               threshold = 1000, compare_method = 'larger', remove_columns = remove_columns, remove_rows = remove_rows, remove_pix = remove_pix,\
                               remove_q_range = {'par':remove_q_par,'ver':remove_q_ver}, \
                               remove_partial_range = {'point_couple':line_strike_segments, 'pixel_width':line_strike_width})
            #update mask and q for l_scan
            peak_fit_engine.mask = mask
            peak_fit_engine.q_oop = grid_q_oop
            peak_fit_engine.q_ip = grid_q_ip
        ...
        #launch peak fitting
        for i in range(1000):
            check_result = peak_fit_engine.reset_fit(img, use_first_fit_for_pos, check = False, tweak_mode = False)
            bkg_sub.center_pix = peak_fit_engine.peak_center
            peak_width_hor_ver = peak_fit_engine.get_peak_width()
            # print(peak_width_hor_ver)
            if bkg_sub.int_direct =='y':
                bkg_sub.fit_background_corr(img*mask,peak_width_hor_ver['ver'],True)
            elif bkg_sub.int_direct =='x':
                bkg_sub.fit_background_corr(img*mask,peak_width_hor_ver['hor'],True)
    ...
    with open(conf_file_plot, 'a+') as f:
        ...
        f.write("phs = [{}]\n".format(ph))
        f.write("colors = ['r']\n")
        f.write("xtal_lattices = ['Co3O4_lat']\n")
