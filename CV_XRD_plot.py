...
which_scans_to_plot = [221]
config_file_name = 'CV_XRD_plot_i20180835_Jul_2019.ini'
...
for scan_id in scan_ids:
    ...
    FWHM_ip, FWHM_oop = data.FWHM_ip, data.FWHM_oop
    ...
    scan_direction_ranges, _, _ = select_cycle((pot,pot),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
    ...
    for pos in range(len(scan_direction_ranges)):
        ...
        if plot_pot_step:
            marker = ''
        #strain data
        _, pot_temp, Time_temp =  select_cycle((pot,Time),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        _, pot_temp, strain_ip_temp =  select_cycle((pot,strain_ip),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        _, pot_temp, sigma_strain_ip_temp =  select_cycle((pot,sigma_strain_ip),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        _, pot_temp, strain_oop_temp =  select_cycle((pot,strain_oop),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        _, pot_temp, sigma_strain_oop_temp =  select_cycle((pot,sigma_strain_oop),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])

        #grain size
        _, pot_temp, FWHM_ip_temp =  select_cycle((pot,FWHM_ip),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        _, pot_temp, FWHM_oop_temp =  select_cycle((pot,FWHM_oop),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        size_ip_temp, size_oop_temp = 0.2*np.pi/FWHM_ip_temp, 0.2*np.pi/FWHM_oop_temp
        #intensity
        _, pot_temp, intensity_temp =  select_cycle((pot,intensity),bin_level = bin_level, plot_mode = ['CV', 'pot_step'][int(bool(plot_pot_step))])
        #x: either potential or time
        x = POT(pot_temp,plot_vs_RHE, scan_info[scan_id].pH) if not plot_pot_step else Time_temp
        ...
        def plot_on_ax(ax,x,data,y_label):
            ...
            if len(x)!=len(data):
                x = x[0:-1]
            ax.plot(x[indx1:indx2],set_max_to_0(data,[indx1,indx2]),linestyle = '-', linewidth =1,\
...
for each in ax_can_set_yticks_ip_strain:
    y_ticks, y_tick_labels = find_tick_ticklables([ip_strain_min+ip_strain_min*offset_scale,-ip_strain_min*offset_scale], num_ticks =5, endpoint = False, dec_place =1)
    each.set_yticks(y_ticks)
    each.set_yticklabels(y_tick_labels)

for each in ax_can_set_yticks_oop_strain:
    y_ticks, y_tick_labels = find_tick_ticklables([oop_strain_min+oop_strain_min*offset_scale,-oop_strain_min*offset_scale], num_ticks =5, endpoint = False, dec_place =1)
    each.set_yticks(y_ticks)
    each.set_yticklabels(y_tick_labels)

for each in ax_can_set_yticks_ip_size:
    y_ticks, y_tick_labels = find_tick_ticklables([ip_size_min+ip_size_min*offset_scale,-ip_size_min*offset_scale], num_ticks =5, endpoint = False, dec_place =1)
    each.set_yticks(y_ticks)
    each.set_yticklabels(y_tick_labels)

for each in ax_can_set_yticks_oop_size:
    y_ticks, y_tick_labels = find_tick_ticklables([oop_size_min+oop_size_min*offset_scale,-oop_size_min*offset_scale], num_ticks =5, endpoint = False, dec_place =1)
    each.set_yticks(y_ticks)
    each.set_yticklabels(y_tick_labels)
