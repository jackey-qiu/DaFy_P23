...
conf_file_names = {'CV_XRD':'config_p23_template.ini',\
                   'ploter':'CV_XRD_plot_i20180835.ini'}
...
for scan_no in scan_nos:
    ...
    if name == 'P23':
        ...
        #UB for first Prep
        #UB = [-2.206,-0.0562,0.00159,-0.0004,0.007229,-0.88946,-1.20879,-2.515,-0.00259]
        #UB for second Prep
    ...
    q_hor_cen, q_ver_cen = q_ip[0, cen[0]], q_oop[cen[1],0]
    ...
    cen = [cen[0]-clip_boundary['hor'][0], cen[1]-clip_boundary['ver'][0]]
    #print(cen)
    ...
    #plt.pcolormesh(grid_q_ip, grid_q_oop, grid_intensity_init,vmax=1000)
    #plt.show()
    ...
    #print("oop",grid_q_oop[cen[1],cen[0]])
    #print("ip",grid_q_ip[cen[1],cen[0]])
    #prim_beam_pot = [np.argmin(abs(np.array(grid_q_oop[:,0])-1.3)),np.argmin(abs(np.array(grid_q_ip[0,:])-2.1))]
    #prim_beam_pot = [np.argmin(abs(np.array(grid_q_oop[:,0])-3.57)),np.argmin(abs(np.array(grid_q_ip[0,:])-2.5))]
    #print(prim_beam_pot)
    #plt.pcolormesh(grid_q_ip, grid_q_oop, grid_intensity_init*mask,vmax=1000)
    #plt.show()
    #prim_beam_pot = [np.argmin(abs(np.array(grid_q_oop[:,0])-2.25)),np.argmin(abs(np.array(grid_q_ip[0,:])-3.28))]
    ...
    def find_boundary(n_process,n_jobs,rank):
        ...
        return left,right
    print(img_loader.total_frame_number)
    if size_cluster == 1:
    ...
    for frame_number in frame_number_ranges:
        ...
        try:
            ...
            check_result = False
        t1_c = time.time()
        #print("current_peak_center",peak_fit_engine.peak_center,"previous peak center",peak_fit_engine.previous_peak_center)
        ...
        if tweak_mode:
            ...
            if live_image:
                ...
                plt.pause(10.05)
        t1_d = time.time()
        #print(t1_d - t1_c, t1_c - t1_b, t1_b - t1_a)
    if not mpi_run:
        plt.ioff()
        plt.show()
    if not live_image:
