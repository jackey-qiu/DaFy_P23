...
import numpy as np
from util.UtilityFunctions import remove_abnormality,
                                  nexus_image_loader,
                                  get_UB,
                                  gen_find,
                                  edf_image_loader, 
                                  check_abnormality,
                                  find_boundary,
                                  extract_global_vars_from_string
...
#mpi run? You will need a proper config file for the mpi run
mpi_run =False 
if mpi_run:
    comm=MPI.COMM_WORLD
...
#'mpi' config is the file for mpi run

conf_file_names = {'CV_XRD':'config_p23_template.ini',\
                   'ploter':'CV_XRD_plot_i20180835_Sep05_2019.ini',\
...
for scan_no in scan_nos:
    ...
    for key in data_keys:
        data[key]=[]
    #id for data saving
    scan_id =  'DataBank_CVXRD_{}_scan{}_{}'.format(beamtime,scan_no,time.strftime("%Y%m%d-%H%M%S"))
    ...
    ########initiate spec file and image loader###########
    if name == "P23":
        #spec_file is not needed if data are stored in nexus file format
    ...
    #set bounds according to the image center
    ...
    #you may need to change the threshold value accordingly#
    ...
    #initiate the peak fitting engine#
    try:
    ...
    #now let's get rid of data points during peak jump area(set check_abnormality to False if beam is stable)
    ...
    ##calculate the potential by fitting the real potential
    ##The calculated potential will be used as a reference in plotting step later
    ...
    for frame_number in frame_number_ranges:
        ...
        #recip mapping
        if l_scan:#this step is time consuming for recal q, but only for L_scan (not recommended for processing L scan data)
            if name == 'P23':
    ...
    with open(conf_file_plot, 'a+') as f:
        try:
            with open(conf_file_plot) as ff:
                pass
        except:
            f.write("[beamtime]\n beamtime = {}\n".format(beamtime))
        f.write('\n[{}]\n'.format(scan_id))
    ...
    #show the big figure of all processed data for this scan at the end 
