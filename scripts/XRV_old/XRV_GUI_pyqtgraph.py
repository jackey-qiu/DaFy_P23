...
class MyMainWindow(QMainWindow):
    def __init__(self, parent = None):
        ...
        pg.mkQApp()
        #load ui script
        uic.loadUi(os.path.join(DaFy_path,'scripts','XRV_old','XRV_bkg_pyqtgraph_new.ui'),self)
    ...
    def setup_image(self):
        ...
        def updatePlot_roi_changed():
            ...
            #show values for current status
            self.lcdNumber_intensity.display(self.app_ctr.data['peak_intensity'][-1])
        roi.sigRegionChanged.connect(updatePlot_roi_changed)
    ...
    def plot_(self):
        ...
        if self.stop:
            ...
            if self.app_ctr.bkg_sub.img is not None:
                ...
                self.region_cut_hor.setRegion(cut_values_ver)
                self.region_cut_ver.setRegion(cut_values_hoz)
    ...
    def reset_peak_center_and_width(self):
        ...
        # roi_size = [int(each/2) for each in self.roi.size()]
        # roi_pos = [int(each) for each in self.roi.pos()]
        # new_center = [roi_pos[0]+roi_size[0],roi_pos[1]+roi_size[1]]
