...
import numpy as np
from util.UtilityFunctions import nexus_image_loader,nexus_image_loader_diffabs, get_UB,gen_find,edf_image_loader
...
conf_file_names = [ "config_diffabs.ini","config_p23.ini","config_id03_MA4171.ini","CV_XRD_plot_temp.ini","config_id03_ch5314.ini","config_id03_ma3886.ini"]
conf_file = os.path.join(DaFy_path, 'config', conf_file_names[0])
conf_file_plot = os.path.join(DaFy_path, 'config', conf_file_names[3])
...
for scan_no in scan_nos:
    ...
    if name == 'P23':
        ...
        img_loader = nexus_image_loader_diffabs(nexus_path = img_path, frame_prefix = frame_prefix)
    ...
    if name == 'P23':
        ...
        motor_angles = img_loader.motor_angles
    ...
    try:
        ...
        # print(grid_q_ip[0][cen[1]])
        # print(grid_q_oop[:,0][cen[0]], grid_q_oop[:,0][cen[1]])
        # print(guess_ip[0], guess_oop[0])
        plt.pcolormesh(grid_q_ip, grid_q_oop, grid_intensity_init*mask)
        # plt.pcolormesh(grid_q_ip, grid_q_oop, img)
        # plt.imshow(img)
        plt.show()
    ...
    # print("total image number",img_loader.total_frame_number)
    for frame_number in frame_number_ranges:
        print("Fitting frame_{} now...".format(frame_number))
        #load image
        try:
            if name=="DIFFABS":
                img = img_loader.load_frame(scan_no, frame_number, offset={'delta':16})
            else:
                img = img_loader.load_frame(scan_no, frame_number, offset={})
        except:
            break
        if name == 'ID03':
            img = img.img
        ...
        if l_scan:#this step is time consuming for recal q, but only for L_scan
            if name == 'P23':
                ...
                motor_angles = img_loader.motor_angles
                r_map.E_keV = img_loader.current_energy
            # motor_angles = spec.extract_motor_angle(motor_angles, scan_no, frame_number, updated_angles =['mu','delta','gamma','omega_t'])
        ...
        for i in range(1000):
            ...
            if i==0:
                ...
                if name == 'P23':
                    ...
                    potential, current = frame_number, 0
