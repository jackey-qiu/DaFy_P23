...
conf_file_names = {'CV_XRD':'config_id03_MA4171.ini',\
                   'ploter':'CV_XRD_plot_temp.ini',\
                   'bkg_sub':'config_bkg_sub.ini'}
...
conf_file_bkg_sub = os.path.join(DaFy_path, 'config', conf_file_names['bkg_sub'])
...
for scan_no in scan_nos:
    ...
    if live_image:
        ...
        fig_bkg_plot = plt.figure(figsize=(5,8))
    ...
    for frame_number in frame_number_ranges:
        ...
        # print(peak_width_hor_ver,bkg_sub.center_pix)
        # quit()
        if bkg_sub.int_direct =='y':
            if live_image:
                bkg_sub.fit_background(fig_bkg_plot,img*mask,peak_width_hor_ver['ver'],live_image)
            else:
                bkg_sub.fit_background(None,img*mask,peak_width_hor_ver['ver'],live_image)
        elif bkg_sub.int_direct =='x':
            if live_image:
                bkg_sub.fit_background(fig_bkg_plot,img*mask,peak_width_hor_ver['hor'],live_image)
            else:
                bkg_sub.fit_background(None,img*mask,peak_width_hor_ver['hor'],live_image)
        peak_fit_engine.update_peak_intensity(bkg_sub.fit_results['I'])
        ...
        if tweak_mode:
            ...
            # quit()
            if bkg_sub.int_direct =='y':
                bkg_sub.fit_background(img*mask,peak_width_hor_ver['ver'],live_image)
            elif bkg_sub.int_direct =='x':
                bkg_sub.fit_background(img*mask,peak_width_hor_ver['hor'],live_image)
            peak_fit_engine.update_peak_intensity(bkg_sub.fit_results['I'])
        else:
            if live_image:
                fig_3_plot = show_3_plots(fig_3_plot,peak_fit_engine.img,peak_fit_engine.grid_q_ip,peak_fit_engine.grid_q_oop,\
    ...
    if rank == 0:
        ...
        #save data
        np.savez('data/%s.npz'%(scan_id),potential=data['potential'], peak_intensity = data['peak_intensity'],\
